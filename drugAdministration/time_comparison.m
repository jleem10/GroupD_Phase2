%
%   compare computation time for SolveODE (in MATLAB script), mexode (C++),
%   and mexodestiff (C++)
%
%   -----------------------------------------------------------------------
%
...
for i = 1:numTrials
    ...
    startT = tic;
    [time, x] = mexode(ModelParameters, [0, 30], InitialConditions, options);
    calcTimes(i) = toc(startT);
    
end

average = mean(calcTimes)
...
for i = 1:numTrials
    ...
    startT = tic;
    SolveODE_simple(InitialConditions);
    calcTimes(i) = toc(startT);
