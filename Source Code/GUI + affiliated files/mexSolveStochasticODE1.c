...
/*************************************************************************/
/* This Program solves a system of 12 coupled ODEs using the Euler -     */
...
/*                      options.stepSize                                 */
/*                                                                       */
/* noiseData        -   A 12x2 double array containing noise parameters  */
...
#include "mexSolveStochasticODE1.h"
...
void mexFunction(int nrhs, mxArray *prhs[], int nlhs, const mxArray *plhs[])
{
    ...
    double D;
    double x[12], dx[12], xNew[12]; // To do: malloc these
    ...
    if(mDimNoise != 2)
    {
        mexErrMsgTxt("need a 12 by 2 array with noise values\n");
        return;
    }
...
//      mexPrintf("Iterations: %d\n", iterations);
    ...
    {
        c_3 = 1 / params.t_peak;
        c_2 = (pow(c_3,2)*pow(params.t_half,2) - 4*c_3*params.t_half + 1) / params.t_half;
        c_1 = params.d_max * (2*c_3 + c_2);
    }
    ...
    for( j=1 ; j<=iterations ; j++) // The main loop
    {
        ...
        dV = (params.gamma_V * I) - (c_1*stepSize/(1+ c_2*stepSize + pow(c_3,2)*pow(stepSize,2))) * params.gamma_V * I - (params.gamma_VA * S * A * V) - (params.gamma_VH * H * V) - (params.alpha_V * V) - (params.a_V1 * V)/(1 + params.a_V2 * V);
        ...
        dP  = (params.b_PB * Bi * P) + (params.a_P)*(1 - P);
        dA  = (params.b_A * P) - (params.gamma_AV * S * A * V) - (params.a_A * A);
        dS  = (params .r*P)*(1 - S);
        dTV = params.kPT - params.kDT * TV * M;
        dBi = -params.kDB * Bi + (params.kPB) * ((TV * Bi) / (TV + Bi));
