...
% Y(11) = TV        T cell clone
% Y(12) = Bi        Activated B cell clone
%
% Y(13)= D          Proportion of Dead Cells
%
...
function [ Solution ] = SolveODE(InitialConditions, ProgramParameters, ModelParameters, NoiseParameters)
...
if ProgramParameters.SDE == 1 && ProgramParameters.mex == 1
    fprintf('Please choose one mode');
elseif ProgramParameters.SDE == 1
options.stepSize = 0.01;
[Solution.t, Solution.Y] = mexSolveStochasticODE1(ModelParameters,...
    [ProgramParameters.t_start,ProgramParameters.t_end],...
    InitialConditions, options, NoiseParameters);

elseif ProgramParameters.mex == 1
    % Can choose stepSize if necesary, but 0.01 is good default value
    options.stepSize = 0.01;
    [Solution.t,Solution.Y] = mexodestiff1(ModelParameters,...
...
Solution.Y(:,13) = 1 - Solution.Y(:,2) - Solution.Y(:,6) - Solution.Y(:,3);
...
dYdt(8,1)  = (ModelParameters.b_PB * M * P) + ...
             (ModelParameters.a_P)*(1 - P);
...
dYdt(12,1) = (-ModelParameters.kDB * Bi) + (ModelParameters.kPB) ...
             * ((TV * Bi) / (TV + Bi));
