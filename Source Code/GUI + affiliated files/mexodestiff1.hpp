...
// % -------------------------------------------------------------------------
// %
// %   A Program to solve a system of ODEs first proposed by Hancioglu et 
// %   al. in the paper 'A Dynamical Model of Human Immune Response to 
...
struct StiffSystem
{
    ...
    void operator()( const VectorType &x, VectorType &dxdy, double /* t */ )
    {
        ...
        dxdy[0]  = (params.gamma_V * I) - (c_1*0.01/(1+ c_2*0.01 + pow(c_3,2)*pow(0.01,2))) * params.gamma_V * I - (params.gamma_VA * S * A * V) - (params.gamma_VH * H * V) - (params.alpha_V * V) - (params.a_V1 * V)/(1 + params.a_V2 * V);
        ...
        dxdy[7]  = (params.b_PB * Bi * P) + (params.a_P)*(1 - P);
        dxdy[8]  = (params.b_A * P) - (params.gamma_AV * S * A * V) - (params.a_A * A);
        dxdy[9]  = (params .r*P)*(1 - S);
        dxdy[10] = (params.kPT) - params.kDT * TV * M;
        dxdy[11] = -params.kDB * Bi + (params.kPB) * ((TV * Bi) / (TV + Bi));
    }
};
...
struct StiffSystemJacobian
{
    ...
    void operator()(const VectorType & x , MatrixType &J , const double & /* t */ , VectorType &dfdt)
    {
        ...
        J(Vi, Vi) = -params.gamma_VA * S * A - params.gamma_VH * H - params.alpha_V - (params.a_V1/((1 + params.a_V2 * V)*(1 + params.a_V2 * V)));
        ...
        J(Pi, Hi) = 0;
        J(Pi, Ii) = 0;
        J(Pi, Mi) = params.b_PB * P;
        J(Pi, Fi) = 0;
        J(Pi, Ri) = 0;
        J(Pi, Ei) = 0;
        J(Pi, Pi) = params.b_PB * M + params.a_P;
        J(Pi, Ai) = 0;
        J(Pi, Si) = 0;
        J(Pi, TVi) = 0;
        J(Pi, Bii) = params.b_PB * Bi;
        ...
        J(TVi, Hi) = 0;
        J(TVi, Ii) = 0;
        J(TVi, Mi) = (params.kPT - params.kDT) * TV;
        J(TVi, Fi) = 0;
        J(TVi, Ri) = 0;
        J(TVi, Ei) = 0;
        J(TVi, Pi) = 0;
        J(TVi, Ai) = 0;
        J(TVi, Si) = 0;
        J(TVi, TVi) = (params.kPT - params.kDT) * M;
        J(TVi, Bii) = 0;
        ...
        J(Bii, Ai) = 0;
        J(Bii, Si) = 0;
        J(Bii, TVi) = -params.kDB + params.kPB * ((TV * TV)/((TV+Bi)*(TV+Bi)));
        J(Bii, Bii) = params.kPB * ((Bi * Bi)/((TV+Bi)*(TV+Bi)));
