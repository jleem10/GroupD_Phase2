...
%
%SDE solver: A stochastic differential equation solver has been written in
% C to solve for stochasticity in the model. This is called within the
% SolveODE function given that the SDE option has been toggled on.
% Associated Noise Parameters (handles.NoiseParameters) can be adjusted by
% the GUI.
%
...
% Last Modified by GUIDE v2.5 17-Jan-2013 21:35:38
...
handles.axis1_xaxis = 15;   % Time from popupmenu
...
handles.ModelParameters.b_PB        = 11.5;
...
handles.ModelParameters.kPT         = 0.2; % Initial values from pg 8, part 1 of Carneiro %
handles.ModelParameters.kDT         = 0.15; % Initial values from pg 8, part 1 of Carneiro %
handles.ModelParameters.kPB         = 0.3; % Initial values from pg 8, part 1 of Carneiro %
handles.ModelParameters.kDB         = 0.1; % Initial values from pg 8, part 1 of Carneiro %
handles.ModelParameters.d_max       = 4;  %----CHANGE----%
handles.ModelParameters.t_peak      = 0.14;  %----CHANGE----%
handles.ModelParameters.t_half      = 0.33;  %----CHANGE----%

%%% Noise Parameters can be input into the GUI
handles.NoiseParameters = zeros(2,12);
handles.NoiseParameters(1,1) = 1; 
handles.NoiseParameters(2,1) = 0.5; 
handles.NoiseParameters(1,8) = 1;
handles.NoiseParameters(2,8) = 0.5;
...
handles.ProgramParameters.SDE = 0;              % option for using SDE solver instead of ODE15s/MEX ODE
...
handles.InitialConditions(11)   = handles.InitialConditions(1);      % TV(0)
handles.InitialConditions(12)   = handles.InitialConditions(1);      % Bi(0)
...
% --- Executes SDE Solver
function checkbox_SDE_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_SDE (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.ProgramParameters.SDE = get(hObject,'Value');
guidata(hObject,handles);
