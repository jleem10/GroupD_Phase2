...
for Index=1:handles.ProgramParameters.nsweeps
   ...
   % drug function needs three parameters c_1, c_2, c_3 whch we extract
   % from the handles struct.
   % First make sure we're not dividing by zero...
   
    if (handles.ModelParameters.t_peak == 0)
        c_1 = 0; c_2 = 0; c_3 = 0;
    else
    ...
    % Finally calculate drug concentrations and add them to the results
    % manifold...
    
    t = results(:,12);
    results(:,13) = c_1*t./(1+c_2*t+c_3^2*t.^2);      %Add drug values to Y matrix
